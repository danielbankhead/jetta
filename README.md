...
### jetta.domainLib `OBJECT`
  ...
  - jetta.domainLib.domainInOtherDomain(`child` STRING, `parent` STRING)
    - returns `true` if `child` is a subdomain or exact match for the `parent`
    - returns `false` in all other cases

    - `child` STRING
      - the domain to see if in `parent`
    - `parent` STRING
      - the parent domain
...
### jetta.makeNestedDirectory `FUNCTION`
  ...
  - jetta.makeNestedDirectory(`path` STRING[, `options` OBJECT])
    - `path` STRING
      - path (full or relative) to create
    - `options` OBJECT
      - `preferredErrorLanguage` STRING
        - The preferred error language as an [ISO 639-1 code](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes)
...
### jetta.JettaError `CLASS` extends `Error`
  ...
  - new jetta.JettaError(`code` STRING, `preferredErrorLanguage` STRING[, `details` OBJECT])
    ...
    - _instance_ OBJECT
      ...
      - `details` OBJECT
        - provides additional details to an error, samples:
      ...
      - `lang` STRING
        - The preferred error language as an [ISO 639-1 code](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes)
      - `message` STRING
        - The localized message in the language matching _instance_.lang
...
### jetta.requestPromise `FUNCTION`
  ...
  - Since Promises cannot return multiple parameters, any errors generated will have a `results` attribute
