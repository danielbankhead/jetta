...
class CookieManager extends events {
  ...
  addCookie (cookie = null, givenOptions = {}) {
    ...
    if (isDeleteCookie === true) {
      ...
      process.nextTick(() => {
        if (updatedExistingCookie === true) {
          this.emit('updatedCookie', Object.assign({domain: cDomain, path: cPath, name: cName}, newCookie))
        } else {
    ...
    return Object.assign({domain: cDomain, path: cPath, name: cName}, newCookie)
  }
  ...
  deleteCookie (cookieObject = {name: '', domain: '', path: ''}) {
    ...
    foundMatch = true
    deletedCookieObject = Object.assign({domain: d, path: p, name: n}, this.cookies[d][p][n])
  ...
  getCookie (cookieObject = {name: '', domain: '', path: ''}) {
    ...
    return Object.assign({domain: d, path: p, name: n}, this.cookies[d][p][n])
  }
