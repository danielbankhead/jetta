...
function errorVerification (state = {t: () => {}, scope: [], e: new jetta.JettaError(), errorCategory: '', preferredErrorLanguage: ''}) {
  ...
  if (errorCategory === 'jetta-make-directory') {
    t.true(/^jetta-make-directory/.test(e.code), em(`error code starts with 'jetta-make-directory'`))
    t.true(e.message.toLowerCase().includes('directory'), em(`error message should contain the word 'directory' to help with context.`))
  }
  ...
  t.equal(typeof e.details, 'object', em(`error.details should be an object`))
